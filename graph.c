...
{
    TipoPeso P[*num_cidades];

    TipoValorVertice Pos[*num_cidades + 1];
    long Antecessor[*num_cidades + 1];
    short Itensheap[*num_cidades + 1];
    TipoVetor A;
    ...
    *soma = 0;
    ...
    matriz_distancia = memAlloc(*num_cidades, 1);
    {
        for(i = 0; i < *num_cidades; i++)
        {
            matriz_distancia[i][0] = P[i];
            *soma = *soma + P[i];
        }
    }
